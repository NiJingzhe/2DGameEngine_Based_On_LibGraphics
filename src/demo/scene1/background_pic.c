...
static void CALLBACK switchBackgroundPic(HWND a, UINT b, UINT_PTR c, DWORD d);
...
void createBackgroundPic()
{
    ...
    switchTimer = newTimer(1, 500, switchBackgroundPic);
...
static void backgroundUpdate(ActorNode background, double delta)
{
    for (int i = 0; i < 255; ++i){
        if (inmng.keyStates['R']){
            if (inmng.keyBoardEventType == BUTTON_DOWN){
